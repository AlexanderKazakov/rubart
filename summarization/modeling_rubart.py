...
"""Copy. Small edits by AlexanderKazakov to incorporate deeppavlov/rubert as BART encoder"""
import logging
import random
...
def _prepare_rubart_decoder_inputs(
...
class PretrainedRuBartModel(PreTrainedModel):
...
class RuBartEncoder(nn.Module):
    ...
    def __init__(self, config: BartConfig, embed_tokens):
        ...
        self.layernorm_embedding = LayerNorm(embed_dim, eps=config.layer_norm_eps)
...
class RuBartModel(PretrainedRuBartModel):
    ...
    def __init__(self, config: BartConfig):
        ...
        self.encoder = RuBartEncoder(config, self.shared)
        self.decoder = RuBartDecoder(config, self.shared)
    ...
    def forward(
        ...
        if not generation_mode:
            decoder_input_ids, decoder_padding_mask, causal_mask = _prepare_rubart_decoder_inputs(
                self.config,
                input_ids,
...
class RuBartForConditionalGeneration(PretrainedRuBartModel):
    """The BART Model with a language modeling head. Can be used for summarization."""
    base_model_prefix = "model"

    def __init__(self, config: BartConfig):
        super().__init__(config)
        base_model = RuBartModel(config)
